...
class AFNO2D(nn.Module):
    def __init__(self, hidden_size, num_blocks=8, sparsity_threshold=0.01, hard_thresholding_fraction=1, hidden_size_factor=1):
        ...
        self.w1 = nn.Parameter(self.scale * torch.randn(2, self.num_blocks,
                               self.block_size, self.block_size * self.hidden_size_factor))
    ...
    def forward(self, x):
        ...
        o1_imag = torch.zeros([B, H, W // 2 + 1, self.num_blocks,
                              self.block_size * self.hidden_size_factor], device=x.device)
...
class AFNONet(nn.Module):
    def __init__(
        ...
        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size=self.patch_size, in_chans=self.in_chans, embed_dim=embed_dim)
        ...
        self.head = nn.Linear(embed_dim, self.out_chans *
                              self.patch_size[0]*self.patch_size[1], bias=False)
...
if __name__ == "__main__":
    ...
    model = AFNONet(img_size=(192, 96), patch_size=(
        16, 16), in_chans=128, out_chans=128)
    sample = torch.randn(1, 128, 192, 96)
    result = model(sample)
    print(result.shape)
