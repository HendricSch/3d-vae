...
class WeightedRMSE():
    ...
    def __call__(self, truth: np.ndarray, forecast: np.ndarray) -> np.ndarray:
        ...
        if len(truth.shape) < 3:
            raise ValueError("Input arrays must have at least two dimensions and one feature channel (c x longitude x latitude) or mini-batched (batchsize x c x longitude x latitude).")
        
        if len(truth.shape) == 3:
            _, _, lat = truth.shape[0], truth.shape[1], truth.shape[2]

        if len(truth.shape) == 4:
            _, _, _, lat = truth.shape[0], truth.shape[1], truth.shape[2], truth.shape[3]

        if lat != self.num_latitudes:
            raise ValueError(f"Number of latitudes ({lat}) must match the number of latitudes used to calculate the weights ({self.num_latitudes}).")
    ...
    def _get_lat_weights(self, num_latitudes: int = 121) -> np.ndarray:
        weights = self._cell_area_from_latitude(np.deg2rad(np.linspace(-90, 90, num_latitudes)))
    ...
    def _spatial_average(self, x: np.ndarray) -> np.ndarray:
        ...
        if len(x.shape) < 3:
            raise ValueError("Input array must have at least two dimensions and one feature channel (c x longitude x latitude) or mini-batched (batchsize x c x longitude x latitude).")
